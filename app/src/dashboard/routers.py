        ...
        if tags:
            ...
            for key,value in stat_dicts.items():
                ...
                if dashboard_crud.check_label_by_file(db=db,filename=filename,label_id=value["label_id"]):
                    ...
                    count -= 1
            return "update dashboard successfully"
        else:
            return "No tag in this file"
...
def get_stat(tagset_id,db:db_dependency):
    ...
    for stat in checked_document:
        ...
        if my_label[0].label_parent != "ROOT":
            ...
            data.append({
                "root_name":my_label[0].label_name,
                "data" : child_data
            })
    ...
    for item in data:
        ...
        child_data = item.get("data")
    
        if root_name in combined_by_root:
            # if combined_by_root[root_name]
            count = 0
            i = combined_by_root[root_name]
            for j in range(len(i)):
                if i[j]['child_name'] == child_data['child_name']:   
                    combined_by_root[root_name][j]['count'] += 1
                    count +=1
                    
            if count != 1:
                combined_by_root[root_name].append(child_data)    
            print(combined_by_root)
        
        else:
            combined_by_root[root_name] = [child_data]
            print(combined_by_root)

    for root_name, data_list in combined_by_root.items():
